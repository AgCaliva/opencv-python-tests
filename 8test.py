...
class Scanner:
  def __init__(self):
    ...
    self.gaussBlurSigmaX = 3
    self.dilateIterations = 3
    self.contourPixWidth = 3
    self.needMorphClose = True

  def setNeedMorphClose(self,needBool):
    self.needMorphClose = needBool

  def setContourWidth(self,nWidth):
    self.contourPixWidth = nWidth

  def setDilateIter( self, dilateIterations ):
    self.dilateIterations = dilateIterations

  def setThresholdCanny( self, thresholdCannyMin , thresholdCannyMax ):
    self.thresholdCannyMin = thresholdCannyMin
    self.thresholdCannyMax = thresholdCannyMax
  
  def setTestPath(self,testpath):
    self.testpath = testpath
  
  def setGaussBlurSigmaX(self,sigmaX):
    self.gaussBlurSigmaX = sigmaX
  #shared funcs:
  def cannyDetect_OtsuMethod(self,img):
    print("cannyDetect_OtsuMethod")
    high_thresh_canny, thresh_im = cv.threshold(img, 0, 255, cv.THRESH_BINARY + cv.THRESH_OTSU)
    low_thresh_canny = 0.5*high_thresh_canny
    self.setThresholdCanny(low_thresh_canny, high_thresh_canny)
  ...
  def Dilate(self,img,shape):
    ...
    self.im_dilate = cv.dilate(img, kernel_dilate_blank, iterations = self.dilateIterations )
    ...
    if len(contours_internal) > 0:
      for contour in contours_internal:
        ...
        if cv.contourArea(contour) <= 0.0:
          continue
        self.imgOriginal_contours = cv.drawContours(self.imgOriginal_contours, contour, 0, (0,255,0), self.contourPixWidth)
  ...
  def algo1(self,tuple_img_nframe):
    ...
    if not cv.imwrite(img_path, imgToWrite):
     raise Exception("No se pudo guardar imagen")
    #----------------------------------------------------------
    self.cannyDetect_OtsuMethod(self.im_gray)
    #----------------------------------------------------------
  ...
  def GaussBlur(self,img,shape):
    print("GaussBlur")
    img_h, img_w, channels = shape 
    #kernelGauss_h = 5
    kernelGauss_h = 3*3
    self.im_gauss_blur = cv.GaussianBlur(img, (kernelGauss_h, kernelGauss_h), self.gaussBlurSigmaX)

  def Tresh(self,img):
    print("Tresh")
    ret, self.im_tresh = cv.threshold(img, 70, 255, cv.THRESH_BINARY)


  def algo2(self,tuple_img_nframe):
    img, nFrame = tuple_img_nframe
    img_name = ""
    self.Contraste( (img,self.alpha,self.beta) )
    img_path = self.testpath+"/"+img_name+str(self.Contraste.__name__)+"_"+str(nFrame)+".jpg"
    ...
    if not cv.imwrite(img_path, imgToWrite):
     raise Exception("No se pudo guardar imagen")
    #----------------------------------------------------------
    self.GaussBlur(self.im_gray,img.shape)
    img_path = self.testpath+"/"+img_name+str(self.GaussBlur.__name__)+"_"+str(nFrame)+".jpg"
    imgToWrite = self.im_gauss_blur
    if not cv.imwrite(img_path, imgToWrite):
     raise Exception("No se pudo guardar imagen")
    #----------------------------------------------------------
    self.Xor(self.im_gauss_blur)
    img_path = self.testpath+"/"+img_name+str(self.Xor.__name__)+"_"+str(nFrame)+".jpg"
    imgToWrite = self.im_xor
    if not cv.imwrite(img_path, imgToWrite):
     raise Exception("No se pudo guardar imagen")
    #----------------------------------------------------------
    self.cannyDetect_OtsuMethod(self.im_xor)
    #----------------------------------------------------------
    #self.Canny(self.im_gauss_blur)
    self.Canny(self.im_xor)
    img_path = self.testpath+"/"+img_name+str(self.Canny.__name__)+"_"+str(nFrame)+".jpg"
    imgToWrite = self.im_canny
    if not cv.imwrite(img_path, imgToWrite):
     raise Exception("No se pudo guardar imagen")
    #----------------------------------------------------------
    self.Tresh(self.im_canny)
    img_path = self.testpath+"/"+img_name+str(self.Tresh.__name__)+"_"+str(nFrame)+".jpg"
    imgToWrite = self.im_tresh
    if not cv.imwrite(img_path, imgToWrite):
     raise Exception("No se pudo guardar imagen")
    #----------------------------------------------------------
    self.Dilate(  self.im_tresh, img.shape )
    img_path = self.testpath+"/"+img_name+str(self.Dilate.__name__)+"_"+str(nFrame)+".jpg"
    imgToWrite = self.im_dilate
    if not cv.imwrite(img_path, imgToWrite):
     raise Exception("No se pudo guardar imagen")
    ...
    if not cv.imwrite(img_path, imgToWrite):
     raise Exception("No se pudo guardar imagen")
    #----------------------------------------------------------
    self.Contours(self.im_morphgrad2,img)
    img_path = self.testpath+"/"+img_name+str(self.Contours.__name__)+"_"+str(nFrame)+".jpg"
    imgToWrite = self.imgOriginal_contours
    if not cv.imwrite(img_path, imgToWrite):
      raise Exception("No se pudo guardar imagen")
...
def run():
  ...
  imgPath = 'dracma.jpg'
  todoslosframes = []
  todoslosframes.append( readImg(imgPath) )
  nFramesProcesados = 0
  ...
  with multiprocessing.Pool(PROCESSES) as p:
    ...
    scanner.setGaussBlurSigmaX(0.8)
    scanner.setDilateIter(0)
    scanner.setContourWidth(2)
    scanner.setNeedMorphClose(False)
    algoritmo_scanner = scanner.algo2
    list_tuples_img_nframe = []
    lenTodosLosFrames = len(todoslosframes)
    if(lenTodosLosFrames == 0):
      p.close()
      p.join()
      return
    if( lenTodosLosFrames == 1 ):
      #solo analiza una imagen:
      frame,nFrame = ( todoslosframes[0], 0 )
      list_tuples_img_nframe.append( (frame,nFrame) )
      task1_result = p.map_async( algoritmo_scanner
                       ,list_tuples_img_nframe )
      p.close()
      p.join()
      return

    for frame,nFrame in zip( todoslosframes,range( lenTodosLosFrames - 1 ) ):
      list_tuples_img_nframe.append( (frame,nFrame) )
    task1_result = p.map_async( algoritmo_scanner
                       ,list_tuples_img_nframe )
    p.close()
